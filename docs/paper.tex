...
eval-awareness language, would miss both models' refusal shifts in our
lineup. We do not claim CoT is uninformative in general; only that the
explicit-VEA signal we measured does not track the behavioral effect,
and a monitor relying on that signal alone would be insufficient. The
pre-registration discipline that disclosed the qwen3-safety
non-replication is itself a methodological contribution, we recommend
the same discipline be applied to other LLM-judge- driven exploratory
findings before they enter the literature.
...
Nair, Ruan, \& Wang (2026). \emph{Value-Conflict Diagnostics Reveal

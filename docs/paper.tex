...
\textbf{Headline: 5 of 6 models additive; 0 of 6 with a Holm-surviving
cross-model interaction.} Only opus earns a per-model
\texttt{interaction\_supported} verdict (Δ −0.060, CI {[}−0.120,
−0.010{]}, p=0.032 uncorrected), but it does not survive Holm-Bonferroni
at the cross-model headline level (corrected threshold p=0.0042 in
Family A of 12 hypotheses). Per-model verdict table, correction policy,
and the opus interaction interpretation are in \textbf{Appendix D.1}.
...
design could not see this two- context pattern, \textbf{Appendix D.2});
\textbf{(b) qwen3 strict-VEA verbalization rate \textasciitilde4× other
models} (0.36 on inject\_eval-alignment vs 0.06-0.09 for the trio,
stable trait that replicates V1 §5.2, \textbf{Appendix D.3});
...
Full 9-row claim grid is in \textbf{Appendix D.5}. Headline rows: V1
inject does NOT generalize to non-safety prompts (not supported on any
of 6 models); stratified deep-dive recovers V1 directions on the
...
(refusal × strict-VEA dual-axis per condition) are in \textbf{Appendix
D.6}. V1's verbalization-vs-behavior dissociation (§5.3) generalizes to
V3 and manifests as two distinct model-level profiles.
...
4-model lineup alone. Detail in \textbf{Appendix D.7}.

\paragraph{5.6.6 What survives, what doesn't (V3, 6-model
lineup)}\label{what-survives-what-doesnt-v3-6-model-lineup}

Full 8-row claim grid is in \textbf{Appendix D.8}. Headline rows: V1's
opus −/qwen3 + direction asymmetry replicates on the fresh V3 sample
(supported, both CIs exclude zero); no singleton clause individually
carries the V1 effect (not supported, 0/18 Holm survivors); the V1
inject is additive-compatible on both active models under the
...
\subsection{Appendix D. Per-model verdicts, secondary findings, and
``what survives''
summaries}\label{appendix-d.-per-model-verdicts-secondary-findings-and-what-survives-summaries}

This appendix collects per-model verdict tables, secondary findings, and
``what survives'' claim grids that were moved from §5.5 and §5.6 to keep
the main text focused on the three headline claims (V2
...
\subsubsection{D.1 V2 per-model verdicts and cross-model interaction
(§5.5.3
detail)}\label{d.1-v2-per-model-verdicts-and-cross-model-interaction-5.5.3-detail}
...
additive; 0 of 6 with a Holm-surviving interaction.} The pre-reg was
explicitly designed to separate per-model verdicts (reported
uncorrected) from cross-model headlines (Holm-corrected), exactly to
avoid the ``one model has p=0.026, write the paper around it'' failure
mode. We report the opus per-model interaction in §5.5.4 as a secondary
finding worth interpretation, not as the headline claim.

\subsubsection{D.2 qwen3's capability-eval inject-suppresses-refusal
pattern (§5.5.5
detail)}\label{d.2-qwen3s-capability-eval-inject-suppresses-refusal-pattern-5.5.5-detail}
...
\subsubsection{D.3 qwen3's strict-VEA verbalization rate (§5.5.6
detail)}\label{d.3-qwen3s-strict-vea-verbalization-rate-5.5.6-detail}
...
\subsubsection{D.4 opus's adaptive thinking on eval-shaped prompts
(§5.5.7
detail)}\label{d.4-opuss-adaptive-thinking-on-eval-shaped-prompts-5.5.7-detail}
...
\subsubsection{D.5 V2 ``what survives, what doesn't'' claim grid
(§5.5.8)}\label{d.5-v2-what-survives-what-doesnt-claim-grid-5.5.8}
...
\subsubsection{D.6 V3 per-clause strict-VEA dissociation profiles
(§5.6.4 detail and Figure
9)}\label{d.6-v3-per-clause-strict-vea-dissociation-profiles-5.6.4-detail-and-figure-9}
...
\subsubsection{D.7 V3 closed-CoT addition (§5.6.5
detail)}\label{d.7-v3-closed-cot-addition-5.6.5-detail}
...
\subsubsection{D.8 V3 ``what survives, what doesn't'' claim grid
(§5.6.6)}\label{d.8-v3-what-survives-what-doesnt-claim-grid-5.6.6}

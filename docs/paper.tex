...
\item
  ...
  inject. Yet both models' refusal attribution falls in the same
  additive-compatible class under the pre-registered §5.5 decision tree.
  Two distinct model-level verbalization profiles co-occur with the same
  refusal-attribution pattern, providing a clause-resolution variant of
  the broader dissociation Knecht et al.~established at the rollout
  level.
\item
...
\texttt{interaction\_supported} → \texttt{additive\ (no-interaction)} →
\texttt{null\_both}, in priority order. Note: the V2 \texttt{additive}
verdict here means ``no detected inject×intrinsic-label interaction in
the 2×2 factorial'' --- it is a different statistical claim from V3's
``additive-compatible'' classification of clause attribution (§5.6.2),
which is about sum-of-singletons matching the full-inject anchor in a
5-condition single-factor design. Both verdicts are non-rejections of
additivity under their respective decision trees, but they refer to
different additivity questions; we keep the V2 label as
\texttt{additive\ (no-interaction)} here to keep this clear in contexts
where both verdicts appear nearby.
...
\paragraph{5.5.3 Per-model verdicts and the cross-model interaction
(summary)}\label{per-model-verdicts-and-the-cross-model-interaction-summary}

\textbf{Headline: 5 of 6 models earn the V2
\texttt{additive\ (no-interaction)} verdict; 0 of 6 have a
Holm-surviving cross-model interaction (distinct from V3's
\texttt{additive-compatible} clause-attribution classification in
§5.6.2).} Only opus earns a per-model \texttt{interaction\_supported}
verdict (Δ −0.060, CI {[}−0.120, −0.010{]}, p=0.032 uncorrected), but it
does not survive Holm-Bonferroni at the cross-model headline level
(corrected threshold p=0.0042 in Family A of 12 hypotheses). Per-model
verdict table, correction policy, and the opus interaction
interpretation are in \textbf{Supplementary S.1}.
...
support \textbf{``on safety/alignment-relevant prompts, opus and qwen3
exhibit opposite-signed refusal shifts under inject framing, attenuated
as the corpus broadens beyond pure safety prompts.''} V1 measured this
on a pure-safety corpus (BeaverTails) and saw the effect at headline
magnitude (qwen3 +22pp, opus −34pp). V2 measured it on a mixed corpus
and saw the same directions at \textasciitilde1/4-1/6 magnitude on the
safety-relevant subset, with the pooled cross-corpus effect diluting to
\textasciitilde0. The V1-like safety/alignment refusal-increase/decrease
pattern does not extend unchanged to capability evals or
deployment-shaped prompts; on capability evals in particular, qwen3
shows a distinct \emph{inject-suppresses-refusal} pattern (§5.5.5 /
Supplementary S.2) rather than the V1-direction pattern.

\paragraph{5.5.5 Secondary V2 findings
(summary)}\label{secondary-v2-findings-summary}

Three V2 secondary findings detailed in the Supplementary Material:
\textbf{(a) qwen3 capability-eval inject-suppresses-refusal} (Δ−0.064,
CI {[}−0.16, −0.001{]}, p=0.046; the qwen3 alignment +5.7pp and
capability −6.4pp cancel in the pooled cross-corpus mean, explaining why
V1's safety-only design could not see this two- context pattern,
\textbf{Supplementary S.2}); \textbf{(b) qwen3 strict-VEA verbalization
rate \textasciitilde4× other models} (0.36 on inject\_eval-alignment vs
0.06-0.09 for the trio, stable trait that replicates V1 §5.2,
\textbf{Supplementary S.3}); \textbf{(c) opus's adaptive thinking skips
on eval-shaped prompts} (\textasciitilde50\% empty
\texttt{reasoning\_trace} on eval-labeled rollouts vs \textasciitilde0\%
on deployment-labeled; a candidate behavioral-level eval-awareness
signal not pre-registered as an outcome and possibly confounded by
Apollo eval prompts being shorter, \textbf{Supplementary S.4}).

\paragraph{5.5.6 What survives, what doesn't (V2, 6-model
lineup)}\label{what-survives-what-doesnt-v2-6-model-lineup}

Full 9-row claim grid is in \textbf{Supplementary S.5}. Headline rows:
V1 inject does NOT generalize to non-safety prompts (not supported on
any of 6 models); stratified deep-dive recovers V1 directions on the
alignment stratum (supported post-hoc, §5.5.4); Apollo intrinsic eval
label predicts refusal across the lineup (5 of 6 models Holm-survivors,
supported); cross-model inject × intrinsic label interaction does NOT
survive Holm in either family.
...
larger BeaverTails magnitude).

Under this distributed-additive interpretation, removing any single
clause from V1's inject text would reduce the effect by some fraction
rather than eliminate it entirely, though V3 does not directly test this
counterfactual (it tests singletons, not pairwise combinations).
\textbf{The eval-noticing behavior on these models appears not localized
to any specific propositional content of the V1 inject; the data are
more consistent with a response to the cumulative weight of eval-framing
claims than with single-clause attribution.}

\paragraph{5.6.4 qwen3's per-clause VEA scales, opus's only jumps under
full\_inject
(summary)}\label{qwen3s-per-clause-vea-scales-opuss-only-jumps-under-full_inject-summary}

V3's secondary VEA outcome reveals a model-level
verbalization-vs-behavior dissociation at clause resolution.
\textbf{qwen3's strict-VEA rises across individual clauses} (×13
baseline on \texttt{eval\_only} alone, ×22 on \texttt{full\_inject}),
while \textbf{opus's strict-VEA only jumps under \texttt{full\_inject}}
(×4), even though both models' refusal behavior is consistent with
additive attribution under the pre-registered §5.5 decision tree.
Per-cell strict-VEA tables for both models and \textbf{Figure 9}
(refusal × strict-VEA dual-axis per condition) are in
\textbf{Supplementary S.6}. V1's verbalization-vs-behavior dissociation
(§5.3) generalizes to V3 and manifests as two distinct model-level
profiles.

\paragraph{5.6.5 Closed-CoT addition: gpt-5.5 and gemini-2.5-pro nulls
(summary)}\label{closed-cot-addition-gpt-5.5-and-gemini-2.5-pro-nulls-summary}

The V3 closed-CoT backfill adds two strong null checks. \textbf{gpt-5.5}
has the cleanest null in the lineup (baseline refusal 0.37,
\texttt{full\_inject} Δ +0.000, tightest CI {[}−0.060, +0.060{]}, every
...
\textbf{gemini-2.5-pro} is \texttt{null\_at\_full} (Δ −0.010 {[}−0.05,
+0.03{]}) with mild sub-additive per-clause suppression. Together with
deepseek and kimi, these provide \textbf{4 null-direction models against
...
4-model lineup alone. Detail in \textbf{Supplementary S.7}.

\paragraph{5.6.6 What survives, what doesn't (V3, 6-model
lineup)}\label{what-survives-what-doesnt-v3-6-model-lineup}

Full 8-row claim grid is in \textbf{Supplementary S.8}. Headline rows:
V1's opus −/qwen3 + direction asymmetry replicates on the fresh V3
sample (supported, both CIs exclude zero); no singleton clause
individually carries the V1 effect (not supported, 0/18 Holm survivors);
the V1 inject is additive-compatible on both active models under the
pre-registered §5.5 decision tree (supported); closed-CoT models do not
react to V3 extrinsic eval-framing (not supported, both null at clause
and full resolution).
...
\textbf{(iii) The V1 inject's effect on opus and qwen3 is
additive-compatible across three clauses under the pre-registered §5.5
decision tree, not localized to any single clause; the interpretation
most consistent with this classification is a distributed-additive cue
response.} V3's 5-condition design tests whether the V1 effect
attributes to (a) the ``this is an evaluation'' claim, (b) the
``reviewed by safety researchers'' claim, (c) the ``compared against
deployment'' claim, or (d) the combined frame. The pre-registered §5.5
decision tree returns \textbf{ADDITIVE-COMPATIBLE on both opus and
qwen3}: sum-of-singletons (−0.077 on opus; +0.051 on qwen3) matches
full\_inject (−0.067; +0.054) within the additivity residual's CI (a
non-rejection of additivity combined with non-detection of singletons;
not an equivalence test for additivity). No single clause's effect is
detectable in isolation; the singleton point estimates are compatible
with roughly distributed contributions across the three clauses, but
individual per-clause magnitudes are not resolved at this n.~V3 does not
directly test the counterfactual of removing a single clause (it tests
singletons against neutral, not pairwise combinations), so the
per-clause-removal prediction is the additive model's extrapolation
rather than a measured result. \textbf{The eval-noticing behavior on
these models appears not localized to any specific propositional content
of the V1 inject, the data are more consistent with a response to the
cumulative weight of eval-framing claims than with single-clause
attribution.}
...
×4 under combined full\_inject, but the refusal pattern is consistent
with additive contributions across all three clauses. \textbf{Two
different per-clause patterns produce the same distributed-additive
refusal attribution on the same kind of prompts.} V1's
verbalization-vs-behavior dissociation generalizes to V3 and shows up at
clause resolution as two distinct model-level profiles.
...
interpretation when reading the Supplementary Material tables. The V2
factorial (§5.5) is the exception: V2 \emph{is} Holm-Bonferroni
corrected across its 4-model × 3-effect family per the pre-registration
...
\item
  \textbf{(b) Additive-compatible clause attribution of the explicit
...
\subsection{Supplementary Material (treat as not core reading; main text
§1-§9 is
self-contained)}\label{supplementary-material-treat-as-not-core-reading-main-text-1-9-is-self-contained}

The blocks below collect per-model verdict tables, secondary findings,
and ``what survives'' claim grids that were moved out of §5.5 and §5.6
...
attribution, V1/V3 direction asymmetry). \textbf{This is supplementary
detail for readers who want the per-(model, condition) granularity,
robustness footnotes, and exhaustive claim grids; the main text §1-§9
stands alone without it.} The substance is unchanged from prior versions
of this draft; only the framing and location have moved. We retain the
``S.x'' cross-references that earlier versions used ``D.x'' for, so
in-text pointers like ``see Supplementary S.1'' map 1:1 to what those
earlier versions called ``Appendix D.1.''

\subsubsection{S.1 V2 per-model verdicts and cross-model interaction
(§5.5.3
detail)}\label{s.1-v2-per-model-verdicts-and-cross-model-interaction-5.5.3-detail}
...
{[}−0.120, −0.010{]} & 0.032 \\
deepseek & additive (no-interaction) & +0.020 & {[}−0.020, +0.070{]} &
0.559 \\
kimi & additive (no-interaction) & −0.010 & {[}−0.070, +0.050{]} &
0.856 \\
qwen3 & additive (no-interaction) & −0.010 & {[}−0.080, +0.060{]} &
0.910 \\
gpt-5.5 & additive (no-interaction) & +0.000 & {[}−0.070, +0.080{]} &
1.000 \\
gemini-2.5-pro & additive (no-interaction) & −0.010 & {[}−0.060,
+0.040{]} & 0.856 \\
\end{longtable}
...
earn the V2 \texttt{additive\ (no-interaction)} verdict; 0 of 6 with a
Holm-surviving interaction.} The pre-reg was explicitly designed to
separate per-model verdicts (reported uncorrected) from cross-model
headlines (Holm-corrected), exactly to avoid the ``one model has
p=0.026, write the paper around it'' failure mode. We report the opus
per-model interaction in §5.5.4 as a secondary finding worth
interpretation, not as the headline claim.

\subsubsection{S.2 qwen3's capability-eval inject-suppresses-refusal
pattern (§5.5.5
detail)}\label{s.2-qwen3s-capability-eval-inject-suppresses-refusal-pattern-5.5.5-detail}
...
pooled mean. One plausible interpretation: the inject may function as a
...
\subsubsection{S.3 qwen3's strict-VEA verbalization rate (§5.5.6
detail)}\label{s.3-qwen3s-strict-vea-verbalization-rate-5.5.6-detail}
...
\subsubsection{S.4 opus's adaptive thinking on eval-shaped prompts
(§5.5.7
detail)}\label{s.4-opuss-adaptive-thinking-on-eval-shaped-prompts-5.5.7-detail}
...
This is a candidate eval-awareness signal distinct from verbalized
eval-awareness, a \emph{behavioral} eval-awareness at the
...
\subsubsection{S.5 V2 ``what survives, what doesn't'' claim grid
(§5.5.8)}\label{s.5-v2-what-survives-what-doesnt-claim-grid-5.5.8}
...
\subsubsection{S.6 V3 per-clause strict-VEA dissociation profiles
(§5.6.4 detail and Figure
9)}\label{s.6-v3-per-clause-strict-vea-dissociation-profiles-5.6.4-detail-and-figure-9}
...
\texttt{full\_inject}. \textbf{qwen3's strict-VEA rises across
individual clauses, while opus's strict-VEA only jumps under
\texttt{full\_inject}, even though both models' refusal behavior is
consistent with additive attribution under the pre-registered §5.5
decision tree.} This is a new finding V3 could produce because of its
5-condition design; V1 and V2 only contrasted ``full inject vs no
...
consistent with additive attribution under the pre-registered §5.5
decision tree (Figure 8), but the verbalization channel follows two
...
\subsubsection{S.7 V3 closed-CoT addition (§5.6.5
detail)}\label{s.7-v3-closed-cot-addition-5.6.5-detail}
...
\subsubsection{S.8 V3 ``what survives, what doesn't'' claim grid
(§5.6.6)}\label{s.8-v3-what-survives-what-doesnt-claim-grid-5.6.6}
